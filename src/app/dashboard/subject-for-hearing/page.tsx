...
const NewIncidentList = () => {
  ...
  const fetchStudents = async (page: number) => {
    try {
      ...
      const { data, error, count } = await supabase
        .from("sti_dgoms_case")
        .select("*", { count: "exact" })
        .eq("caseTracking", "Subject for Hearing")
        .range(start, end);
      ...
      if (data) {
        const formattedStudents = data.map((student: any) => ({
          ...
          subjectForHearing: student.subjectForHearing,
  ...
  const renderRow = (item: Student) => (
    ...
    >
      ...
      <td className="md:table-cell">{item.subjectForHearing}</td>
      ...
      <td>
        <div className="flex items-center gap-2">
          <>
            <Link href={`/dashboard/subject-for-hearing/${item.id}`}>
