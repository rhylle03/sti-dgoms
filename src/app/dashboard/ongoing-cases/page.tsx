...
const columns = [
  ...
  },
  {
    ...
    accessor: "caseHearingStartDate",
    className: "md:table-cell",
  },
  {
    header: "Hearing End Date",
    accessor: "caseHearingEndDate",
    className: "md:table-cell",
  },
  {
    header: "Status",
...
const NewIncidentList = () => {
  ...
  const getStatus = (startDate: string) => {
    ...
    const hearingStartDate = new Date(startDate);

    if (hearingStartDate <= currentDate) {
      return "Ongoing";
    } else {
      const diffInMs = hearingStartDate.getTime() - currentDate.getTime();
  ...
  const renderRow = (item: Student) => (
    ...
    >
      ...
      <td className="md:table-cell font-semibold">{item.caseNumber}</td>
      <td className="md:table-cell">{formatDate(item.caseHearingStartDate)}</td>
      <td className="md:table-cell">{formatDate(item.caseHearingEndDate)}</td>
      <td className="md:table-cell">
        {getStatus(item.caseHearingStartDate)}
      </td>{" "}
      <td>
        <div className="flex items-center gap-2">
          <Link href={`/dashboard/ongoing-cases/${item.id}`}>
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 mt-0">
      <div className="flex items-center justify-between">
        <h1 className="md:block text-lg font-semibold">Ongoing cases </h1>
