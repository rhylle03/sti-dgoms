...
export default function GoodMoralRequestDialog({ fullName }: any) {
  ...
  const handleSubmit = async () => {
    ...
    const appointmentDateTime = formatISO(
      parseISO(`${appointmentDate}T${appointmentTime}`)
    );

    try {
      const { error: errorgmr } = await supabase
        .from("good_moral_requests")
        .insert([
          {
            student_name: fullName,
            request_status: "open",
            appointment_schedule: appointmentDateTime,
          },
        ]);
      if (errorgmr) {
        console.error("Error updating good moral request:", errorgmr);
        return;
      }
