...
type ConsultationData = {
  ...
  appointment_schedule_start: Date;
  appointment_schedule_end: Date;
  student_name: string;
  created_at: string;
};
      ...
      {showDropdown && (
        <div className="absolute right-0 mt-2 w-64 bg-white border border-gray-300 rounded-lg shadow-lg overflow-hidden z-50">
          <ul className="divide-y divide-gray-200">
            ...
            ) : notifications.length > 0 || consultationData.length > 0 ? (
              <>
                <div className="">
                  <p className="p-3 font-bold text-md">
                    <span className="flex gap-1 items-center">
                      <CircleAlert size={14} /> Important Notifications
                    </span>
                  </p>
                  {consultationData
                    .filter((consultation) =>
                      dayjs(consultation?.appointment_schedule_start).isBefore(
                        dayjs().add(24, "hour")
                      )
                    )
                    .map((consultation) => (
                      <li key={consultation?.id} className="px-3 mb-2">
                        <p className="text-sm font-bold text-red-600">
                          Consultation
                        </p>
                        <p className="text-xs text-gray-500">
                          You have a consultation meeting
                        </p>
                        <p className="text-xs text-gray-500">
                          Time:{" "}
                          {dayjs(
                            consultation?.appointment_schedule_start
                          ).format("MM/DD/YY h:mm A")}{" "}
                          -{" "}
                          {dayjs(consultation?.appointment_schedule_end).format(
                            "h:mm A"
                          )}
                        </p>
                      </li>
